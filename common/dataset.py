...
class DeepfakeDataset(Dataset):
    ...
    def __init__(self, paths_file, image_size, id, n_c_samples=None, val=False):
        ...
        self.transform_train_edge = A.Compose([
            ...
            # specially for edge mask, as the paper uses H/4 and W/4
            A.HorizontalFlip(),
            A.VerticalFlip(),
            ToTensorV2()
        ])
    ...
    def __getitem__(self, item):
        ...
        if (not self.val):
            ...
            #shape = edge.shape
            #edge = edge.reshape([1, shape[0], shape[1]])
...
if __name__ == "__main__":
    ...
    img = plt.imread("./datas/train/train/tampered/imgs/0000.jpg")
    mask = cv2.imread("./datas/train/train/tampered/masks/0000.png", cv2.IMREAD_GRAYSCALE)
    plt.subplot(1, 4, 1)
    plt.imshow(img)
    plt.subplot(1, 4, 2)
    plt.imshow(mask)
